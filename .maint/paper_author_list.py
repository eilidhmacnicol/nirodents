...
if __name__ == '__main__':
    ...
    print(
        'Some people made commits, but are missing in .maint/ files: ',
        '{", ".join(unmatched)}.',
        file=sys.stderr,
    )

    print(f'Authors ({len(author_matches):d}):')
    print(
        '; '.join(
            [f'{i["name"]} {idx}' for i, idx in zip(author_matches, aff_indexes, strict=True)]
        )
    )
